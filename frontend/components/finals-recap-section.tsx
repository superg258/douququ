...
export function FinalsRecapSection() {
  ...
  const load = useCallback(async (signal: AbortSignal) => {
    ...
    if (!events.repechage || !events.nationals) throw new Error("决赛数据不完整");
    const repechage = events.repechage.event;
    const nationals = events.nationals.event;
    const nextRecap = !hasOfficialFinalMatchData(repechage) && !hasOfficialFinalMatchData(nationals)
      ? buildFinalsPredictionRecap(null, { repechage, nationals })
      : buildFinalsPredictionRecap(
        simulateFinalsLiveEvents(repechage, nationals, overview, DEFAULT_SEED),
        { repechage, nationals },
      );
    setRecap(nextRecap);
