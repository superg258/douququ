...
function toneClass(connector: CanvasConnector) {
  ...
  const dashed = strong ? "" : "stroke-dasharray-[4_5]";
  switch (connector.tone) {
    case "amber":
      return strong
        ? "stroke-rm-result-winner opacity-100"
        : `stroke-rm-result-winner opacity-25 ${dashed}`;
    case "emerald":
      return strong
        ? "stroke-rm-status-safe opacity-100"
        : `stroke-rm-status-safe opacity-25 ${dashed}`;
    case "steel":
      return `stroke-white/15 opacity-25 ${dashed}`;
    default:
      return strong ? "stroke-rm-blue opacity-100" : `stroke-rm-metal-text opacity-18 ${dashed}`;
  }
}
...
export function CanvasConnectorView({
  ...
  const strokeWidth = connector.weight === "strong" ? "stroke-[3.5px]" : "stroke-[2.5px]";
