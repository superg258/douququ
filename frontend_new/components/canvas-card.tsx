...
function MatchCanvasCardComponent({
  ...
  const isCompleted = mode === "sim" ? true : Boolean(row.isRealResult);
  ...
  const containerBorder = (() => {
    if (mode === "sim") return "border-[2px] border-rm-status-safe shadow-[0_0_20px_rgba(0,255,157,0.3)] bg-rm-status-safe/10";
    if (row.isRealResult) {
  ...
  const statusConfig = (() => {
    if (mode === "sim") return { label: "推演战果", className: "border-rm-status-safe font-extrabold text-rm-status-safe bg-rm-status-safe/20 shadow-[0_0_12px_rgba(0,255,157,0.6)]" };
    if (row.isRealResult) {
      const predWinnerSame = (predictedScore.scoreline[0] > predictedScore.scoreline[2]) === (redGames > blueGames);
      const predScoreSame = predictedScore.scoreline === row.scoreline;
      ...
      <div className={cn("flex flex-col flex-1 p-2 relative z-10 w-full h-full", card.orderLabel ? "pl-8" : "")}>
        ...
        <div className="flex flex-1 items-stretch justify-between relative bg-[#0a0a0f] border border-rm-metal-border/30 clip-chamfer min-h-[60px] mb-1.5 group-hover:border-rm-metal-border/70 transition-colors">
          ...
          <div className={cn(
            ...
            isCompleted && actualWinnerName === row.redTeam.collegeName ? "border-rm-status-safe shadow-[inset_0_0_12px_rgba(0,255,157,0.15)]" : "border-rm-red",
          ...
          onClick={(e) => { e.stopPropagation(); onTeamSelect(row.redTeam.teamKey); }}>
            {isCompleted && actualWinnerName === row.redTeam.collegeName && (
              <span className="text-[7.5px] font-machine text-rm-status-safe tracking-widest mb-0.5 animate-pulse">{">>> WINNER"}</span>
            )}
            <div 
              title={row.redTeam.collegeName} 
              className={cn("text-[13px] leading-tight font-bold tracking-widest break-normal line-clamp-2 mt-0.5 h-full flex items-center shadow-black drop-shadow-md", isCompleted && actualWinnerName === row.redTeam.collegeName ? "text-white text-glow-white" : "text-rm-red")}
            >
          ...
          <div className="flex-[0.16] flex flex-col items-center justify-center relative shrink-0 z-20">
            ...
            {isCompleted ? (
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#0a0a0f] border border-rm-status-safe px-2 py-0.5 text-lg font-machine text-rm-status-safe shadow-[0_0_10px_rgba(0,255,157,0.4)] whitespace-nowrap text-glow">
                {row.scoreline}
              </div>
          ...
          <div className={cn(
            ...
            isCompleted && actualWinnerName === row.blueTeam.collegeName ? "border-rm-status-safe shadow-[inset_0_0_12px_rgba(0,255,157,0.15)]" : "border-rm-blue",
          ...
          onClick={(e) => { e.stopPropagation(); onTeamSelect(row.blueTeam.teamKey); }}>
            ...
            <div 
              ...
              className={cn("text-[13px] leading-tight font-bold tracking-widest break-normal line-clamp-2 mt-0.5 h-full flex items-center justify-end shadow-black drop-shadow-md", isCompleted && actualWinnerName === row.blueTeam.collegeName ? "text-white text-glow-white" : "text-rm-blue")}
            >
