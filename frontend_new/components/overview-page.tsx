...
function TacticalRosterGrid({ teams }: { teams: OverviewTeam[] }) {
  ...
  return (
    <div className="mt-8 border-t border-rm-metal-border pt-4 -mx-4 px-4 bg-rm-metal-dark/20 backdrop-blur-sm relative z-20">
      <div className="flex items-center mb-4">
        ...
        <h4 className="text-[10px] font-bold text-rm-blue tracking-widest uppercase text-glow-blue">
          TACTICAL ROSTER DATA GRID
        </h4>
      </div>
      
      <div className="w-full max-h-[320px] overflow-y-auto no-scrollbar scroll-smooth relative">
...
function RegionCard({ region }: { region: RegionDashboardCard }) {
  ...
  const hasMockRealtime = region.regionSlug === "south_region";
  const mockRealtimeSeed = 20261111;
  const realtimeBadge = hasMockRealtime ? "已接入" : "待接入";
  const realtimeHint = hasMockRealtime
    ? "南部赛区已写入瑞士轮前两轮已完赛数据，可直接进入实时胜率预测与赛后复盘。"
    : "当前模块尚未接入真实赛中数据；此处为真实信息入口预留位，不属于赛程模拟内容。";
  ...
  return (
    ...
    >
      <div>
        <div
          className={cn(
            "mb-4 bg-gradient-to-r via-rm-metal-dark px-3 py-2.5 text-white",
            hasMockRealtime
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="flex min-w-0 flex-col">
              <span className="text-[9px] font-mono uppercase tracking-[0.2em] text-rm-metal-text">Realtime Entry</span>
          ...
          {hasMockRealtime ? (
            <div className="mt-3 flex items-center justify-between gap-2">
              <span className="text-[10px] font-mono text-rm-metal-text/80">
                进度锚点：按已完赛场次实时回传
              </span>
              <Link
                href={buildRegionHref(region.regionSlug, "swiss-a", { seed: mockRealtimeSeed })}
                className="shrink-0 border border-rm-status-safe/45 bg-rm-status-safe/12 px-2.5 py-1 text-[10px] font-bold tracking-widest text-rm-status-safe hover:bg-rm-status-safe/20"
              >
                进入实时胜率预测
              </Link>
            </div>
        ...
        <div className="flex items-center justify-between border-b-2 border-rm-metal-border pb-2 mb-4">
        ...
        <div className="mt-4 mb-2 p-3 bg-rm-metal-dark/50 border-l-2 border-rm-blue/50 text-[11px] text-rm-metal-text leading-relaxed font-mono">
      ...
      {isLive && (
        <div className="mt-auto border-t border-rm-metal-border pt-4 -mx-4 px-4 flex justify-between gap-2 overflow-x-auto pb-1 no-scrollbar">
...
function SystemBrief() {
  return (
    <MechCard label="SYSTEM BRIEF / 赛区监控网络简报" className="mb-6">
      <div className="text-sm font-mono text-rm-metal-text leading-relaxed space-y-2">
        ...
        <p><span className="text-rm-blue/80 font-bold tracking-widest text-xs uppercase animate-pulse">{">>>"} SYSTEM DATA SYNCHRONIZED</span></p>
      </div>
    </MechCard>
  );
}
